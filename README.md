...
- **Rate limiting is fail-open and IP-based.** It now covers **login, register, and forgot-password**, but still has **no per-account lockout** and is **only enforced when `REDIS_URL` is set** (no Redis → no limiting). Consider per-account throttling and ensuring Redis is provisioned in production.
- **Stateless JWT staleness is bounded, not eliminated.** Session `maxAge` is **7 days** (down from the 30-day default), and the guards now re-read the DB: `requireUser` rejects a deleted account, `requireAdmin` rejects a demoted admin on writes, and the `(admin)` layout re-checks the role so an ex-admin can't even *view* `/admin`. But a *promoted* user still won't gain admin until their next login/token refresh, and a **password change does not revoke existing tokens** until they expire — a `tokenVersion`-style claim would close that. These are inherent to the stateless-JWT strategy.
- **Orphaned blobs from abandoned/failed gallery uploads.** Images upload to Blob *as files are picked*; if a multi-image upload fails mid-batch, or the admin uploads then **abandons the form without saving**, those objects were never written to the DB, so the post-commit reconcile never sees them → they leak. (Saved-then-removed images **are** cleaned up.) A client-side cleanup-on-cancel or a periodic orphan sweep would address it.
- **AtlasEntry delete does not clean up its Blob.** It predates `deleteBlobs`, so deleting an atlas entry leaves its image object in Blob storage — a known, minor leak. (Questions and articles **do** clean up, now host-filtered + reference-counted so a URL shared by two records survives deleting one.) Worth aligning AtlasEntry with the same post-commit cleanup.
- **Image origin allow-list is wide open.** `next.config.ts` allows `remotePatterns: [{ protocol: "https", hostname: "**" }]`. Admin-only authoring caps the practical risk, but tightening to the specific Blob host would be safer. (Blob *deletion* is already scoped to the Vercel Blob host in `deleteBlobs`.)
- **Rate-limit counter is INCR-then-EXPIRE (not atomic).** If the process dies between the two, the key never gets a TTL and that IP's bucket stays "full" until manually cleared. Low probability; a `SET … EX … NX` + `INCR` pair (or a Lua script) would make it atomic.
